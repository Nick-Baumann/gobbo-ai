...
import com.nickbaumann.gobbo.node.NodeForegroundService
import com.nickbaumann.gobbo.node.VoiceWakeMode
  ...
  LazyColumn(
      ...
      AnimatedVisibility(visible = voiceWakeMode != VoiceWakeMode.Off) {
        Column(verticalArrangement = Arrangement.spacedBy(6.dp), modifier = Modifier.fillMaxWidth()) {
          ListItem(
            ...
            supportingContent = { Text("Listens only while Gobbo is open.") },
            trailingContent = {
              RadioButton(
    ...
    item {
      Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
        Button(
          onClick = {
            val parsed = com.nickbaumann.gobbo.node.WakeWords.parseCommaSeparated(wakeWordsText)
    ...
    item {
      ListItem(
        ...
        supportingContent = { Text("Keeps the screen awake while Gobbo is open.") },
