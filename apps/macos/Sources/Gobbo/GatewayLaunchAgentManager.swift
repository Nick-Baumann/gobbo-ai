...
enum GatewayLaunchAgentManager {
    private static let logger = Logger(subsystem: "com.nickbaumann.gobbo", category: "gateway.launchd")
    ...
    private static func gatewayExecutablePath(bundlePath: String) -> String {
        "\(bundlePath)/Contents/Resources/Relay/gobbo"
    }
    ...
    private static func writePlist(bundlePath: String, port: Int) {
        ...
        var envEntries = """
            ...
            <key>GOBBO_IMAGE_BACKEND</key>
            <string>sips</string>
        """
        if let token {
            envEntries += """
                <key>GOBBO_GATEWAY_TOKEN</key>
                <string>\(token)</string>
            """
        }
        let plist = """
    ...
    private static func preferredGatewayToken() -> String? {
        let raw = ProcessInfo.processInfo.environment["GOBBO_GATEWAY_TOKEN"] ?? ""
