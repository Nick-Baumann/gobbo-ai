...
actor BridgeServer {
    ...
    private let logger = Logger(subsystem: "com.nickbaumann.gobbo", category: "bridge")
    ...
    func start() async {
        ...
        do {
            ...
            listener.start(queue: DispatchQueue(label: "com.nickbaumann.gobbo.bridge"))
...
enum BridgePairingApprover {
    static func approve(request: BridgePairRequest, isRepair: Bool) async -> Bool {
        await withCheckedContinuation { cont in
            ...
            let alert = NSAlert()
            alert.messageText = isRepair ? "Re-pair Gobbo Node?" : "Pair Gobbo Node?"
