...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.gobbo/gobbo.json. Prefer GOBBO_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func loadConfig() {
        let gateway = GobboConfigFile.loadGatewayDict()
    ...
    private func applySettings() {
        ...
        GobboConfigFile.updateGatewayDict { gateway in
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.gobbo/gobbo.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.gobbo/gobbo.json. Restart the gateway to apply."
        }
