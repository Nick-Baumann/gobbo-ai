...
enum CommandResolver {
    private static let projectRootDefaultsKey = "gobbo.gatewayProjectRootPath"
    private static let helperName = "gobbo"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager.default.isReadableFile(atPath: distEntry) { return distEntry }
        let binEntry = root.appendingPathComponent("bin/gobbo.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/gobbo")
    ...
    static func gobboExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectGobboExecutable(projectRoot: URL? = nil) -> String? {
    ...
    static func nodeCliPath() -> String? {
        let candidate = self.projectRoot().appendingPathComponent("bin/gobbo.js").path
        return FileManager.default.isReadableFile(atPath: candidate) ? candidate : nil
    }

    static func hasAnyGobboInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.gobboExecutable(searchPaths: searchPaths) != nil { return true }
        if self.findExecutable(named: "pnpm", searchPaths: searchPaths) != nil { return true }
        if self.findExecutable(named: "node", searchPaths: searchPaths) != nil, self.nodeCliPath() != nil { return true }
        return false
    }

    static func gobboNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            ...
            if let gobboPath = self.projectGobboExecutable(projectRoot: root) {
                return [gobboPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "gobbo", subcommand] + extraArgs
            }
            if let gobboPath = self.gobboExecutable(searchPaths: searchPaths) {
                return [gobboPath, subcommand] + extraArgs
            }

            let missingEntry = """
            gobbo entrypoint missing (looked for dist/index.js or bin/gobbo.js); run pnpm build.
            """
    ...
    // Existing callers still refer to gobboCommand; keep it as node alias.
    static func gobboCommand(
        subcommand: String,
        extraArgs: [String] = [],
        defaults: UserDefaults = .standard,
        searchPaths: [String]? = nil) -> [String]
    {
        self.gobboNodeCommand(subcommand: subcommand, extraArgs: extraArgs, defaults: defaults, searchPaths: searchPaths)
    }
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real gobbo CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
            ...
            "/usr/sbin",
            "/sbin",
            "/Users/nickbaumann/Library/pnpm",
            "$PATH",
        ].joined(separator: ":")
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/gobbo"
        ...
        \(cliSection)
        \(projectSection)
        if command -v gobbo >/dev/null 2>&1; then
          CLI="$(command -v gobbo)"
          gobbo \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/gobbo.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/gobbo.js"
            node "$PRJ/bin/gobbo.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent gobbo"
          pnpm --silent gobbo \(quotedArgs);
        else
          echo "gobbo CLI missing on remote host"; exit 127;
        fi
        """
    ...
    static func connectionSettings(defaults: UserDefaults = .standard) -> RemoteSettings {
        ...
        if let modeRaw {
            ...
            let seen = defaults.bool(forKey: "gobbo.onboardingSeen")
