...
private let deepLinkLogger = Logger(subsystem: "com.nickbaumann.gobbo", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "Gobbo is paused", message: "Unpause Gobbo to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run Gobbo agent?", message: body) else { return }
        }
