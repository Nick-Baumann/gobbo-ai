...
final class HealthStore {
    ...
    private static let logger = Logger(subsystem: "com.nickbaumann.gobbo", category: "health")
    ...
    var summaryLine: String {
        ...
        if !snap.web.linked { return "Not linked — run gobbo login" }
    ...
    var detailLine: String? {
        if let error = self.lastError, !error.isEmpty {
            ...
            if lower.contains("connection refused") {
                return "The gateway control port (127.0.0.1:18789) isn’t listening — restart Gobbo to bring it back."
            }
    ...
    func describeFailure(from snap: HealthSnapshot, fallback: String?) -> String {
        if !snap.web.linked {
            return "Not linked — run gobbo login"
        }
        if let connect = snap.web.connect, !connect.ok {
