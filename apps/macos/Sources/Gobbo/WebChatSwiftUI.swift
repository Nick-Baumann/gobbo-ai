import AppKit
import GobboChatUI
import GobboProtocol
import Foundation
import OSLog
import QuartzCore
import SwiftUI

private let webChatSwiftLogger = Logger(subsystem: "com.nickbaumann.gobbo", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: GobboChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> GobboChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(GobboChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> GobboChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                GobboGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<GobboChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any GobboChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = GobboChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: GobboChatView(viewModel: vm, userAccent: accent))
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<GobboChatView>) -> NSViewController
    {
