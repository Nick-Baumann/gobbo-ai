...
enum CLIInstaller {
    private static func embeddedHelperURL() -> URL {
        Bundle.main.bundleURL.appendingPathComponent("Contents/Resources/Relay/gobbo")
    }
        ...
        fileManager: FileManager) -> String?
    {
        ...
        for basePath in searchPaths {
            let candidate = URL(fileURLWithPath: basePath).appendingPathComponent("gobbo").path
    ...
    static func install(statusHandler: @escaping @Sendable (String) async -> Void) async {
        ...
        let targets = cliHelperSearchPaths.map { "\($0)/gobbo" }
