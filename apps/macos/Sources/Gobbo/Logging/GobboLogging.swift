...
enum GobboLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = GobboOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = GobboFileLogHandler(label: label)
    ...
    static func parseLabel(_ label: String) -> (String, String) {
        guard let range = label.range(of: Self.labelSeparator) else {
            return ("com.nickbaumann.gobbo", label)
        }
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        GobboLogging.bootstrapIfNeeded()
        let label = GobboLogging.makeLabel(subsystem: subsystem, category: category)
...
struct GobboOSLogHandler: LogHandler {
...
struct GobboFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = GobboLogging.parseLabel(self.label)
