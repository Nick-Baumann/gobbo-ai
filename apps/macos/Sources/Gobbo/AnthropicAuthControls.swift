...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: GobboOAuthStore.AnthropicOAuthStatus = GobboOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager.default.fileExists(atPath: GobboOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = GobboOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = GobboOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try GobboOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. Gobbo can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: GobboOAuthStore.AnthropicOAuthStatus,
