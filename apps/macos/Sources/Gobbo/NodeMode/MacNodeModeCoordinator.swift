...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "com.nickbaumann.gobbo", category: "mac-node")
    ...
    private func currentCaps() -> [String] {
        var caps: [String] = [GobboCapability.canvas.rawValue, GobboCapability.screen.rawValue]
        if UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false {
            caps.append(GobboCapability.camera.rawValue)
        }
        return caps
    }
    ...
    private func currentCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            GobboCanvasCommand.hide.rawValue,
            GobboCanvasCommand.navigate.rawValue,
            GobboCanvasCommand.evalJS.rawValue,
            GobboCanvasCommand.snapshot.rawValue,
            GobboCanvasA2UICommand.push.rawValue,
            GobboCanvasA2UICommand.pushJSONL.rawValue,
            GobboCanvasA2UICommand.reset.rawValue,
            MacNodeScreenCommand.record.rawValue,
            GobboSystemCommand.run.rawValue,
            GobboSystemCommand.notify.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(GobboCapability.camera.rawValue) {
            commands.append(GobboCameraCommand.snap.rawValue)
            commands.append(GobboCameraCommand.clip.rawValue)
        }
    ...
    static func loopbackBridgePort() -> UInt16? {
        if let raw = ProcessInfo.processInfo.environment["GOBBO_BRIDGE_PORT"],
    ...
    static func probeEndpoint(_ endpoint: NWEndpoint, timeoutSeconds: Double) async -> Bool {
        ...
        connection.start(queue: DispatchQueue(label: "com.nickbaumann.gobbo.macos.bridge-loopback-probe"))
        do {
    ...
    private static func discoverBridgeEndpoint(timeoutSeconds: Double) async -> NWEndpoint? {
        ...
        return await withCheckedContinuation { cont in
            ...
            for domain in GobboBonjour.bridgeServiceDomains {
                let browser = NWBrowser(
                    for: .bonjour(type: GobboBonjour.bridgeServiceType, domain: domain),
                    using: params)
                ...
                browser.start(queue: DispatchQueue(label: "com.nickbaumann.gobbo.macos.bridge-discovery.\(domain)"))
            }
...
enum MacNodeTokenStore {
    private static let suiteName = "com.nickbaumann.gobbo.shared"
