...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "com.nickbaumann.gobbo", category: "gateway.env")
    private static let supportedBindModes: Set<String> = ["loopback", "tailnet", "lan", "auto"]

    static func bundledGatewayExecutable() -> String? {
        guard let res = Bundle.main.resourceURL else { return nil }
        let path = res.appendingPathComponent("Relay/gobbo").path
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.gobboExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expected?.description,
                    message: "gobbo CLI not found in PATH; install the global package.")
            }
    ...
    private static func preferredGatewayBind() -> String? {
        if let env = ProcessInfo.processInfo.environment["GOBBO_GATEWAY_BIND"] {
        ...
        let root = GobboConfigFile.loadDict()
    ...
    static func installGlobal(version: Semver?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "gobbo@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "gobbo@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "gobbo@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "gobbo@\(target)"])
            }

        statusHandler("Installing gobbo@\(target) via \(label)…")
        let response = await ShellExecutor.run(command: cmd, cwd: nil, env: ["PATH": preferred], timeout: 300)
        if response.ok {
            statusHandler("Installed gobbo@\(target)")
        } else {
