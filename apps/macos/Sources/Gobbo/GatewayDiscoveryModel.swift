...
final class GatewayDiscoveryModel {
    ...
    private let logger = Logger(subsystem: "com.nickbaumann.gobbo", category: "gateway-discovery")
    ...
    func start() {
        ...
        for domain in GobboBonjour.bridgeServiceDomains {
            ...
            browser.start(queue: DispatchQueue(label: "com.nickbaumann.gobbo.macos.gateway-discovery.\(domain)"))
        }
    }
    ...
    private nonisolated static func prettifyInstanceName(_ decodedName: String) -> String {
        ...
        let stripped = normalized.replacingOccurrences(of: " (Gobbo)", with: "")
