...
struct ConfigSettings: View {
    ...
    // clawd browser settings (stored in ~/.gobbo/gobbo.json under "browser")
    ...
    // Talk mode settings (stored in ~/.gobbo/gobbo.json under "talk")
    ...
    private var header: some View {
        Text("Gobbo CLI config")
            .font(.title3.weight(.semibold))
        Text("Edit ~/.gobbo/gobbo.json (agent / session / routing / messages).")
            .font(.callout)
    ...
    private var anthropicAuthHelpText: String {
        "Determined from Gobbo OAuth token file (~/.gobbo/credentials/oauth.json) " +
    ...
    private func saveConfig() async {
        ...
        GobboConfigFile.saveDict(root)
    }

    private func loadConfigDict() -> [String: Any] {
        GobboConfigFile.loadDict()
    }
