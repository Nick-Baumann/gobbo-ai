...
actor TalkModeRuntime {
    ...
    private let logger = Logger(subsystem: "com.nickbaumann.gobbo", category: "talk.runtime")
    private let ttsLogger = Logger(subsystem: "com.nickbaumann.gobbo", category: "talk.tts")
    ...
    private func latestAssistantText(sessionKey: String, since: Double? = nil) async -> String? {
        do {
            ...
            let decoded: [GobboChatMessage] = messages.compactMap { item in
                guard let data = try? JSONEncoder().encode(item) else { return nil }
                return try? JSONDecoder().decode(GobboChatMessage.self, from: data)
            }
