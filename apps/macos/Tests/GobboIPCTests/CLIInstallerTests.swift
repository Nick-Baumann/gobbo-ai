...
struct CLIInstallerTests {
    @Test func installedLocationOnlyAcceptsEmbeddedHelper() throws {
        ...
        let root = fm.temporaryDirectory.appendingPathComponent(
            "gobbo-cli-installer-\(UUID().uuidString)")
        defer { try? fm.removeItem(at: root) }

        let embedded = root.appendingPathComponent("Relay/gobbo")
        ...
        let link = binDir.appendingPathComponent("gobbo")
        ...
        let other = root.appendingPathComponent("Other/gobbo")
