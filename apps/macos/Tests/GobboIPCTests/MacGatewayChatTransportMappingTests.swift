...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: GobboProtocol.AnyCodable(["ok": GobboProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: GobboProtocol.AnyCodable(["ok": GobboProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload: GobboProtocol.AnyCodable = GobboProtocol.AnyCodable([
            "runId": GobboProtocol.AnyCodable("run-1"),
            "sessionKey": GobboProtocol.AnyCodable("main"),
            "state": GobboProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: GobboProtocol.AnyCodable(["a": GobboProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
