...
@Suite(.serialized) struct UtilitiesTests {
    ...
    @Test func gatewayEntrypointPrefersDistOverBin() throws {
        ...
        let bin = tmp.appendingPathComponent("bin/gobbo.js")
    ...
    @Test func logLocatorPicksNewestLogFile() throws {
        ...
        let dir = URL(fileURLWithPath: "/tmp/gobbo", isDirectory: true)
        try? fm.createDirectory(at: dir, withIntermediateDirectories: true)

        let older = dir.appendingPathComponent("gobbo-old-\(UUID().uuidString).log")
        let newer = dir.appendingPathComponent("gobbo-new-\(UUID().uuidString).log")
