...
@Suite(.serialized) struct CommandResolverTests {
    ...
    @Test func prefersGobboBinary() async throws {
        ...
        let gobboPath = tmp.appendingPathComponent("node_modules/.bin/gobbo")
        try self.makeExec(at: gobboPath)

        let cmd = CommandResolver.gobboCommand(subcommand: "gateway", defaults: defaults)
        #expect(cmd.prefix(2).elementsEqual([gobboPath.path, "gateway"]))
    }
    ...
    @Test func fallsBackToNodeAndScript() async throws {
        ...
        let scriptPath = tmp.appendingPathComponent("bin/gobbo.js")
        ...
        let cmd = CommandResolver.gobboCommand(
            subcommand: "rpc",
            defaults: defaults,
    ...
    @Test func fallsBackToPnpm() async throws {
        ...
        let cmd = CommandResolver.gobboCommand(subcommand: "rpc", defaults: defaults)

        #expect(cmd.prefix(4).elementsEqual([pnpmPath.path, "--silent", "gobbo", "rpc"]))
    }
    ...
    @Test func pnpmKeepsExtraArgsAfterSubcommand() async throws {
        ...
        #expect(cmd.prefix(5).elementsEqual([pnpmPath.path, "--silent", "gobbo", "health", "--json"]))
    ...
    @Test func buildsSSHCommandForRemoteMode() async throws {
        ...
        defaults.set("/srv/gobbo", forKey: remoteProjectRootKey)

        let cmd = CommandResolver.gobboCommand(subcommand: "status", extraArgs: ["--json"], defaults: defaults)
        ...
        if let script = cmd.last {
            #expect(script.contains("cd '/srv/gobbo'"))
            #expect(script.contains("gobbo"))
