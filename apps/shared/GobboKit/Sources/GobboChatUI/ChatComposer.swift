...
@MainActor
struct GobboChatComposer: View {
    @Bindable var viewModel: GobboChatViewModel
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(GobboChatTheme.composerBackground)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
                        .strokeBorder(GobboChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \GobboPendingAttachment.id)
                { (att: GobboPendingAttachment) in
                    HStack(spacing: 6) {
    ...
    private var editor: some View {
        RoundedRectangle(cornerRadius: 12, style: .continuous)
            .strokeBorder(GobboChatTheme.composerBorder)
            .background(
                RoundedRectangle(cornerRadius: 12, style: .continuous)
                    .fill(GobboChatTheme.composerField))
            .overlay {
                VStack(alignment: .leading, spacing: 4) {
                    self.editorOverlay
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(GobboChatTheme.subtleCard)
        .clipShape(Capsule())
    }
