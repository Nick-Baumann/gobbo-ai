...
private let chatUILogger = Logger(subsystem: "com.nickbaumann.gobbo", category: "GobboChatUI")

@MainActor
@Observable
public final class GobboChatViewModel {
    public private(set) var messages: [GobboChatMessage] = []
    ...
    public var attachments: [GobboPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [GobboChatPendingToolCall] = []
    public private(set) var sessions: [GobboChatSessionEntry] = []
    private let transport: any GobboChatTransport
    ...
    private var pendingToolCallsById: [String: GobboChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any GobboChatTransport) {
    ...
    public func removeAttachment(_ id: GobboPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [GobboChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: GobboChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [GobboChatMessage]) -> [GobboChatMessage] {
        var result: [GobboChatMessage] = []
    ...
    private static func dedupeKey(for message: GobboChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [GobboChatMessageContent] = [
            GobboChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> GobboChatAttachmentPayload in
            GobboChatAttachmentPayload(
                type: att.type,
    ...
    private func handleTransportEvent(_ evt: GobboChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: GobboChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: GobboAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = GobboChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> GobboPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
