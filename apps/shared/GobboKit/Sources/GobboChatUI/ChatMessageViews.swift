...
struct ChatMessageBubble: View {
    let message: GobboChatMessage
    let style: GobboChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? GobboChatTheme.userText : GobboChatTheme.assistantText
            ...
            if !split.images.isEmpty {
                ...
                { (item: ChatMarkdownSplitter.InlineImage) in
                    if let img = item.image {
                        GobboPlatformImageFactory.image(img)
                            .resizable()
                            .scaledToFit()
                            .frame(maxHeight: 260)
    ...
    private var inlineAttachments: [GobboChatMessageContent] {
    ...
    private var toolCalls: [GobboChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [GobboChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? GobboChatTheme.userBubble
        }
        if self.style == .onboarding {
            return GobboChatTheme.onboardingAssistantBubble
        }
        return GobboChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return GobboChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: GobboChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? GobboChatTheme.userText : GobboChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: GobboChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(GobboChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(GobboChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatStreamingAssistantBubble: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ChatMarkdownBody(text: self.text, textColor: GobboChatTheme.assistantText)
        }
        .padding(12)
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [GobboChatPendingToolCall]
