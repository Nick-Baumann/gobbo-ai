...
@MainActor
public struct GobboChatView: View {
    public enum Style {
        case standard
        case onboarding
    }
    ...
    public var body: some View {
        ZStack {
            GobboChatTheme.background
                .ignoresSafeArea()

            VStack(spacing: Layout.stackSpacing) {
                self.messageList
                GobboChatComposer(viewModel: self.viewModel, style: self.style)
            }
    ...
    private var visibleMessages: [GobboChatMessage] {
        let base: [GobboChatMessage]
    ...
    private func mergeToolResults(in messages: [GobboChatMessage]) -> [GobboChatMessage] {
    ...
    private func isToolResultMessage(_ message: GobboChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func toolCallIds(in message: GobboChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: GobboChatMessage) -> String {
