...
export const WIDE_AREA_DISCOVERY_DOMAIN = "gobbo.internal.";
export const WIDE_AREA_ZONE_FILENAME = "gobbo.internal.db";
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*gobbo-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaBridgeZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "gobbo";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "gobbo");
  ...
  records.push(
    `_gobbo-bridge._tcp IN PTR ${instanceLabel}._gobbo-bridge._tcp`,
  );
  records.push(
    `${instanceLabel}._gobbo-bridge._tcp IN SRV 0 0 ${opts.bridgePort} ${hostLabel}`,
  );
  records.push(
    `${instanceLabel}._gobbo-bridge._tcp IN TXT ${txt.map(txtQuote).join(" ")}`,
  );
  ...
  return `; gobbo-content-hash: ${contentHash}\n${contentBody}`;
}
