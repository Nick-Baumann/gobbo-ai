...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a gobbo.internal zone with bridge PTR/SRV/TXT records", () => {
    const txt = renderWideAreaBridgeZoneText({
      serial: 2025121701,
      bridgePort: 18790,
      displayName: "Mac Studio (Gobbo)",
    ...
    expect(txt).toContain(
      `_gobbo-bridge._tcp IN PTR studio-london._gobbo-bridge._tcp`,
    );
    expect(txt).toContain(
      `studio-london._gobbo-bridge._tcp IN SRV 0 0 18790 studio-london`,
    );
    expect(txt).toContain(`displayName=Mac Studio (Gobbo)`);
  });
