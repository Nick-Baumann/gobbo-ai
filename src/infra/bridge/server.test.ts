...
describe("node bridge server", () => {
  ...
  beforeAll(async () => {
    process.env.GOBBO_ENABLE_BRIDGE_IN_TESTS = "1";
    baseDir = await fs.mkdtemp(path.join(os.tmpdir(), "gobbo-bridge-test-"));
  });

  afterAll(async () => {
    await fs.rm(baseDir, { recursive: true, force: true });
    delete process.env.GOBBO_ENABLE_BRIDGE_IN_TESTS;
  });
