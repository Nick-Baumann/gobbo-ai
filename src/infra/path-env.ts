...
function candidateBinDirs(opts: EnsureGobboPathOpts): string[] {
  ...
  // Bun bundled (macOS app): `gobbo` lives in the Relay dir (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingGobbo = path.join(execDir, "gobbo");
    if (isExecutable(siblingGobbo)) candidates.push(execDir);
  } catch {
    // ignore
  }

  // Project-local installs (best effort): if a `node_modules/.bin/gobbo` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "gobbo")))
...
/**
 * Best-effort PATH bootstrap so skills that require the `gobbo` CLI can run
 * under launchd/minimal environments (and inside the macOS bun bundle).
 */
export function ensureGobboCliOnPath(opts: EnsureGobboPathOpts = {}) {
  if (process.env.GOBBO_PATH_BOOTSTRAPPED === "1") return;
  process.env.GOBBO_PATH_BOOTSTRAPPED = "1";
