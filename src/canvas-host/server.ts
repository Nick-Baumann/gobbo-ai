...
(() => {
  ...
  const hasIOS = () => !!(window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.gobboCanvasA2UIAction);
  const hasAndroid = () => !!(window.gobboCanvasA2UIAction && typeof window.gobboCanvasA2UIAction.postMessage === "function");
  const hasHelper = () => typeof window.gobboSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("gobbo:a2ui-action-status", (ev) => {
  ...
  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Gobbo node canvas.");
      return;
    }
    const ok = window.gobboSendUserAction({
      name,
      surfaceId: "main",
      sourceComponentId,
      context: { t: Date.now() },
...
function isDisabledByEnv() {
  if (process.env.GOBBO_SKIP_CANVAS_HOST === "1") return true;
    ...
    try {
      ...
      if (!filePath) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Gobbo Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
