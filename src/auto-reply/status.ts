...
type AgentConfig = NonNullable<GobboConfig["agent"]>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts always live at: ~/.gobbo/sessions/<SessionId>.jsonl
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const webLine = (() => {
    if (args.webLinked === false) {
      return "Web: not linked — run `gobbo login` to scan the QR.";
    }
