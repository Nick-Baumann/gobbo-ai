...
export function registerDnsCli(program: Command) {
  ...
  dns
    .command("setup")
    .description(
      "Set up CoreDNS to serve gobbo.internal for unicast DNS-SD (Wide-Area Bonjour)",
    )
    .option(
      "--apply",
    ...
    .action(async (opts) => {
      ...
      console.log("");
      console.log("Recommended ~/.gobbo/gobbo.json:");
      console.log(
        JSON.stringify(
          {
            bridge: { bind: "tailnet" },
      ...
      console.log(`- Restrict to domain (Split DNS): gobbo.internal`);
      ...
      const serverPath = path.join(confDir, "gobbo.internal.server");
      ...
      if (zoneFileNeedsBootstrap(zonePath)) {
        ...
        const zoneLines = [
          `; created by gobbo dns setup (will be overwritten by the gateway when wide-area discovery is enabled)`,
      ...
      if (cfg.discovery?.wideArea?.enabled !== true) {
        console.log("");
        console.log(
          "Note: enable discovery.wideArea.enabled in ~/.gobbo/gobbo.json on the gateway and restart the gateway so it writes the DNS-SD zone.",
        );
      }
    });
}
