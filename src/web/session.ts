...
export function resolveWebAuthDir() {
  return path.join(os.homedir(), ".gobbo", "credentials");
}
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === DisconnectReason.loggedOut) {
            console.error(
              danger("WhatsApp session logged out. Run: gobbo login"),
            );
          }
        }
...
export async function pickProvider(pref: Provider | "auto"): Promise<Provider> {
  ...
  if (!hasWeb) {
    throw new Error(
      "No WhatsApp Web session found. Run `gobbo login --verbose` to link.",
    );
  }
  return choice;
}
