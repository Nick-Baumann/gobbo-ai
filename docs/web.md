...
- Config (`config.get`, `config.set`) for `~/.gobbo/gobbo.json`
...
```bash
export GOBBO_GATEWAY_TOKEN="…your token…"
gobbo gateway
```
...
```json5
{
  gateway: {
    ...
    auth: { mode: "password" } // or GOBBO_GATEWAY_PASSWORD
  }
}
```
...
- Binding the Gateway to a non-loopback address **requires** auth (`GOBBO_GATEWAY_TOKEN` or `gateway.auth`).
