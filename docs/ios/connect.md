---
summary: "Runbook: connect/pair the iOS node to a Gobbo Gateway and drive its Canvas"
read_when:
...
- You can run the CLI (`gobbo`) on the gateway machine (or via SSH).

## 1) Start the Gateway (with bridge enabled)

Bridge is enabled by default (disable via `GOBBO_BRIDGE_ENABLED=0`).
...
- Set `bridge.bind: "tailnet"` in `~/.gobbo/gobbo.json` on the gateway host.
...
You should see your gateway advertising `_gobbo-bridge._tcp`.

If browse works, but the iOS node can’t connect, try resolving one instance:

```bash
dns-sd -L "<instance name>" _gobbo-bridge._tcp local.
```
...
1) Set up a DNS-SD zone (example `gobbo.internal.`) on the gateway host and publish `_gobbo-bridge._tcp` records.
2) Configure Tailscale split DNS for `gobbo.internal` pointing at that DNS server.
...
- Via Gateway (paired + connected):
  ```bash
  gobbo gateway call node.list --params "{}"
  ```
- Via Gateway presence (legacy-ish, still useful):
  ```bash
  gobbo gateway call system-presence --params "{}"
  ```
...
- `window.__gobbo.canvas`
- `window.__gobbo.ctx` (2D context)
- `window.__gobbo.setStatus(title, subtitle)`
...
```bash
gobbo nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-hostname>.local:18793/__gobbo__/canvas/"}'
```

Notes:
- The server injects a live-reload client into HTML and reloads on file changes.
- A2UI is hosted on the same canvas host at `http://<gateway-host>:18793/__gobbo__/a2ui/`.
- Tailnet (optional): if both devices are on Tailscale, use a MagicDNS name or tailnet IP instead of `.local`, e.g. `http://<gateway-magicdns>:18793/__gobbo__/canvas/`.
...
```bash
gobbo nodes invoke --node "iOS Node" --command canvas.eval --params "$(cat <<'JSON'
{"javaScript":"(() => { const {ctx,setStatus} = window.__gobbo; setStatus('Drawing','…'); ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle='#ff2d55'; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); setStatus(null,null); return 'ok'; })()"}
JSON
)"
```
...
```bash
gobbo nodes invoke --node 192.168.0.88 --command canvas.snapshot --params '{"maxWidth":900}'
```
