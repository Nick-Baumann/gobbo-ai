...
## Goals
- Build an **iOS app** that acts as a **remote node** for Gobbo:
...
- The Gateway WebSocket server binds to `127.0.0.1:18789` (`src/gateway/server.ts`) with an optional `GOBBO_GATEWAY_TOKEN`.
- The Gateway exposes a Canvas file server (`canvasHost`) on `canvasHost.port` (default `18793`), so nodes can `canvas.navigate` to `http://<lanHost>:18793/__gobbo__/canvas/` and auto-reload on file changes (`docs/configuration.md`).
...
### Pairing
- Bonjour discovery shows a candidate “Gobbo Bridge” on the LAN.
- First connection:
  ...
  4) Approval can happen via:
     - **macOS UI** (Gobbo shows an alert with Approve/Reject/Later, including the node IP), or
...
- If the Swift UI is not present: `gobbo` CLI can list pending requests and approve/reject.
...
- A2UI (mobile + macOS canvas):
  ...
  - `canvas.a2ui.reset`
  - A2UI is hosted by the Gateway canvas host (`/__gobbo__/a2ui/`) on `canvasHost.port`. Commands fail if the host is unreachable.
...
```bash
gobbo nodes invoke --node ios-node --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__gobbo; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```
...
- `GobboProtocol` (generated models; platform-neutral)
- `GobboGatewayClient` (shared WS framing + connect/req/res + seq-gap handling)
- `GobboKit` (node/canvas command types + deep links + shared utilities)
...
- Canvas/workspace files (persistent, private):
  - `Application Support/Gobbo/canvas/<sessionKey>/...`
- Snapshots / temp exports (evictable):
  - `Library/Caches/Gobbo/canvas-snapshots/<sessionKey>/...`
- Credentials:
...
- Pairing (source of truth):
  - `~/.gobbo/nodes/paired.json`
  - `~/.gobbo/nodes/pending.json` (or `pending/*.json` for auditability)
