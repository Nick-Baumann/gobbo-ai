...
Gobbo uses Bonjour (mDNS / DNS-SD) as a **LAN-only convenience** to discover a running Gateway bridge transport. It is best-effort and does **not** replace SSH or Tailnet-based connectivity.
...
2) Publish DNS-SD records for `_gobbo-bridge._tcp` in a dedicated zone (example: `gobbo.internal.`).
3) Configure Tailscale **split DNS** so `gobbo.internal` resolves via that DNS server for clients (including iOS).

Gobbo standardizes on the discovery domain `gobbo.internal.` for this mode. iOS/Android nodes browse both `local.` and `gobbo.internal.` automatically (no per-device knob).

### Gateway config (recommended)

On the gateway host (the machine running the Gateway bridge), add to `~/.gobbo/gobbo.json` (JSON5):

```json5
{
  bridge: { bind: "tailnet" }, // tailnet-only (recommended)
  discovery: { wideArea: { enabled: true } } // enables gobbo.internal DNS-SD publishing
}
```
...
```bash
gobbo dns setup --apply
```

This installs CoreDNS and configures it to:
- listen on port 53 **only** on the gateway’s Tailscale interface IPs
- serve the zone `gobbo.internal.` from the gateway-owned zone file `~/.gobbo/dns/gobbo.internal.db`
...
```bash
dns-sd -B _gobbo-bridge._tcp gobbo.internal.
dig @<TAILNET_IPV4> -p 53 _gobbo-bridge._tcp.gobbo.internal PTR +short
```
...
- Add split DNS so the domain `gobbo.internal` uses that nameserver.

Once clients accept tailnet DNS, iOS nodes can browse `_gobbo-bridge._tcp` in `gobbo.internal.` without multicast.
...
- Set `bridge.bind: "tailnet"` in `~/.gobbo/gobbo.json`.
...
Only the **Node Gateway** (`clawd` / `gobbo gateway`) advertises Bonjour beacons.
...
- `_gobbo-bridge._tcp` — bridge transport beacon (used by macOS/iOS/Android nodes).
...
- `canvasPort=<port>` (only when the canvas host is enabled + reachable; default `18793`; serves `/__gobbo__/canvas/`)
- `cliPath=<path>` (optional; absolute path to a runnable `gobbo` entrypoint or binary)
...
- Browse instances:
  - `dns-sd -B _gobbo-bridge._tcp local.`
- Resolve one instance (replace `<instance>`):
  - `dns-sd -L "<instance>" _gobbo-bridge._tcp local.`
...
The iOS node app discovers bridges via `NWBrowser` browsing `_gobbo-bridge._tcp`.
...
- UIs should decode for display (iOS uses `BonjourEscapes.decode` in `apps/shared/GobboKit`).

## Disabling / configuration

- `GOBBO_DISABLE_BONJOUR=1` disables advertising.
- `GOBBO_BRIDGE_ENABLED=0` disables the bridge listener (and therefore the bridge beacon).
- `bridge.bind` / `bridge.port` in `~/.gobbo/gobbo.json` control bridge bind/port (preferred).
- `GOBBO_BRIDGE_HOST` / `GOBBO_BRIDGE_PORT` still work as a back-compat override when `bridge.bind` / `bridge.port` are not set.
- `GOBBO_SSH_PORT` overrides the SSH port advertised in `_gobbo-bridge._tcp`.
- `GOBBO_TAILNET_DNS` publishes a `tailnetDns` hint (MagicDNS) in `_gobbo-bridge._tcp`. If unset, the gateway auto-detects Tailscale and publishes the MagicDNS name when possible.
