...
read_when:
  ...
  - Deciding: standalone CLI vs deep Gobbo integration
...
Recommendation: **deep integration in Gobbo**, but keep a separable core library.

### Why integrate into Gobbo?
- Gobbo already knows:
  - the workspace path (`agent.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `gobbo memory recall "…" --k 25 --since 30d`
  - `gobbo memory reflect --since 7d`
...
### Phase 1: `gobbo memory index|recall` (FTS-only)
