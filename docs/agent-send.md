---
summary: "Design notes for a direct `gobbo agent` CLI subcommand without WhatsApp delivery"
read_when:
  - Adding or modifying the agent CLI entrypoint
---
# `gobbo agent` (direct-to-agent invocation)

`gobbo agent` lets you talk to the **embedded** agent runtime directly (no chat send unless you opt in), while reusing the same session store and thinking/verbose persistence as inbound auto-replies.
