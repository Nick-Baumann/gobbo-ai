...
Add a `groupChat` block to `~/.gobbo/gobbo.json` so display-name pings work even when WhatsApp strips the visual `@` in the text body:
...
- Session store entries will appear as `group:<jid>` in the session store (`~/.gobbo/sessions/sessions.json` by default); a missing entry just means the group hasn’t triggered a run yet.
