...
---
# Gobbo gateway as a child process of the macOS app

Date: 2025-12-06 · Status: draft · Owner: nickbaumann
...
## Goal
Run the Node-based Gobbo/gobbo gateway as a direct child of the LSUIElement app (instead of a launchd agent) while keeping all TCC-sensitive work inside the Swift app/broker layer and wiring the existing “Gobbo Active” toggle to start/stop the child.

## When to prefer the child-process mode
- You want gateway lifetime strictly coupled to the menu-bar app (dies when the app quits) and controlled by the “Gobbo Active” toggle without touching launchd.
...
- Add a small `GatewayProcessManager` (Swift) that owns:
  ...
  - `start(config)` called when “Gobbo Active” flips ON:
    - binary: host Node running the bundled gateway under `Gobbo.app/Contents/Resources/Gateway/`
    - args: current gobbo entrypoint and flags
    - cwd/env: point to `~/.gobbo` as today; inject the expanded PATH so Homebrew Node resolves under launchd
    - output: stream stdout/stderr to `/tmp/gobbo-gateway.log` (cap buffer via Subprocess OutputLimits)
...
- Stream child stdout/stderr to `/tmp/gobbo-gateway.log`; surface the last N lines in the Debug tab.
...
- Respect the existing pause semantics: when paused, the broker should return `ok=false, "gobbo paused"`; the gateway should avoid calling privileged routes while paused.
...
- Implement `GatewayProcessManager` with Swift Subprocess to start/stop the gateway on the “Gobbo Active” toggle.
