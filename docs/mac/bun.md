...
Goal: ship **Gobbo.app** with a self-contained relay binary that can run both the CLI and the Gateway daemon. No global `npm install -g gobbo`, no system Node requirement.

## What gets bundled

App bundle layout:

- `Gobbo.app/Contents/Resources/Relay/gobbo`
  - bun `--compile` relay executable built from `dist/macos/relay.js`
  - Supports:
    - `gobbo …` (CLI)
    - `gobbo gateway-daemon …` (LaunchAgent daemon)
- `Gobbo.app/Contents/Resources/Relay/package.json`
  - tiny “p runtime compatibility” file (see below)
- `Gobbo.app/Contents/Resources/Relay/theme/`
  - p TUI theme payload (optional, but strongly recommended)

Why the sidecar files matter:
- The embedded p runtime detects “bun binary mode” and then looks for `package.json` + `theme/` **next to `process.execPath`** (i.e. next to `gobbo`).
...
Version injection:
- `--define "__GOBBO_VERSION__=\"<pkg version>\""`
- `src/version.ts` also supports `__GOBBO_VERSION__` (and `GOBBO_BUNDLED_VERSION`) so `--version` doesn’t depend on reading `package.json` at runtime.

## Launchd (Gateway as LaunchAgent)

Label:
- `com.nickbaumann.gobbo.gateway`

Plist location (per-user):
- `~/Library/LaunchAgents/com.nickbaumann.gobbo.gateway.plist`

Manager:
- `apps/macos/Sources/Gobbo/GatewayLaunchAgentManager.swift`

Behavior:
- “Gobbo Active” enables/disables the LaunchAgent.
- App quit does **not** stop the gateway (launchd keeps it alive).

Logging:
- launchd stdout/err: `/tmp/gobbo/gobbo-gateway.log`

Default LaunchAgent env:
- `GOBBO_IMAGE_BACKEND=sips` (avoid sharp native addon under bun)
...
- `scripts/codesign-mac-app.sh` signs `Relay/gobbo` with:
...
```bash
dist/Gobbo.app/Contents/Resources/Relay/gobbo --version

GOBBO_SKIP_PROVIDERS=1 \
GOBBO_SKIP_CANVAS_HOST=1 \
dist/Gobbo.app/Contents/Resources/Relay/gobbo gateway-daemon --port 18999 --bind loopback
```

Then, in another shell:

```bash
pnpm -s gobbo gateway call health --url ws://127.0.0.1:18999 --timeout 3000
```
