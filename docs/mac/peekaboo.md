---
summary: "Plan for integrating Peekaboo automation into Gobbo via PeekabooBridge (socket-based TCC broker)"
read_when:
  - Hosting PeekabooBridge in Gobbo.app
  - Integrating Peekaboo as a submodule
  - Changing PeekabooBridge protocol/paths
---
# Peekaboo Bridge in Gobbo (macOS UI automation broker)
...
- Gobbo integrates by **optionally hosting the same bridge** inside **Gobbo.app** (user-toggleable). The primary client is the **`peekaboo` CLI** (installed via npm); Gobbo does not need its own `ui …` CLI surface.
- For **visualizations**, we keep them in **Peekaboo.app** (best UX); Gobbo stays a thin broker host. No visualizer toggle in Gobbo.

Non-goals:
- No auto-launching Peekaboo.app.
- No onboarding deep links from the automation endpoint (Gobbo onboarding already handles permissions).
- No AI provider/agent runtime dependencies in Gobbo (avoid pulling Tachikoma/MCP into the Gobbo app/CLI).

## Big refactor (Dec 2025): XPC → Bridge
Peekaboo’s privileged execution moved from “CLI → XPC helper” to “CLI → socket bridge host”. For Gobbo this is a win:
- It matches the existing “local socket + codesign checks” approach.
- It lets us piggyback on **either** Peekaboo.app’s permissions **or** Gobbo.app’s permissions (whichever is running).
...
- Gobbo: `~/Library/Application Support/gobbo/bridge.sock`

No auto-launch: if a host isn’t reachable, the command fails with a clear error (start Peekaboo.app, Claude.app, or Gobbo.app).
...
What Gobbo should use:
- **Client side**: `PeekabooBridge` (socket client + protocol models).
- **Host side (Gobbo.app)**: `PeekabooBridgeHost` + the minimal Peekaboo services needed to implement operations.

What Gobbo should *not* embed:
...
## IPC / CLI surface
### No `gobbo ui …`
We avoid a parallel “Gobbo UI automation CLI”. Instead:
- `peekaboo` is the user/agent-facing CLI surface for automation and capture.
- Gobbo.app can host PeekabooBridge as a **thin TCC broker** so Peekaboo can piggyback on Gobbo permissions when Peekaboo.app isn’t running.
...
- Gobbo.app converts to global CG coordinates using `NSScreen.screens[screenIndex].frame.origin`.
...
- Gobbo hosts the bridge, but does not render overlays.
- Any “visualizer enabled/disabled” setting is controlled in Peekaboo.app.

## Screenshots (legacy → Peekaboo takeover)
Gobbo should not grow a separate screenshot CLI surface.

Migration plan:
- Use `peekaboo capture …` / `peekaboo see …` (returns a file path, default temp directory).
- Once Gobbo’ legacy screenshot plumbing is replaced, remove it cleanly (no aliases).
...
2. Host `PeekabooBridgeHost` inside Gobbo.app behind a single setting (“Enable Peekaboo Bridge”, default on).
3. Ensure Gobbo hosts the bridge at `~/Library/Application Support/gobbo/bridge.sock` and speaks the PeekabooBridge JSON protocol.
4. Validate with `peekaboo bridge status --verbose` that Peekaboo can select Gobbo as the fallback host (no auto-launch).
