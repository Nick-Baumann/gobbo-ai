---
summary: "macOS IPC architecture for Gobbo app, gateway node bridge, and PeekabooBridge"
read_when:
  - Editing IPC contracts or menu bar app IPC
---
# Gobbo macOS IPC architecture (Dec 2025)

**Current model:** there is **no local control socket** and no `gobbo-mac` CLI. All agent actions go through the Gateway WebSocket and `node.invoke`. UI automation still uses PeekabooBridge.
...
- Host preference order (client-side): Peekaboo.app → Claude.app → Gobbo.app → local execution.
...
- See: `docs/mac/peekaboo.md` for the Gobbo plan and naming.
