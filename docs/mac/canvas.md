...
Gobbo can embed an agent-controlled “visual workspace” panel (“Canvas”) inside the macOS app using `WKWebView`, served via a **custom URL scheme** (no loopback HTTP port required).
...
- `gobbo-canvas://<session>/<path>`

Routing model:
- `gobbo-canvas://main/` → `<canvasRoot>/main/index.html` (or `index.htm`)
- `gobbo-canvas://main/yolo` → `<canvasRoot>/main/yolo/index.html` (or `index.htm`)
- `gobbo-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
...
- `~/Library/Application Support/Gobbo/canvas/<session>/…`

This keeps it alongside other app-owned state and avoids mixing with `~/.gobbo/` gateway config.
...
This should be modeled after `WebChatManager`/`WebChatSwiftUIWindowController` but targeting `gobbo-canvas://…` URLs.

Related:
- For “invoke the agent again from UI” flows, prefer the macOS deep link scheme (`gobbo://agent?...`) so *any* UI surface (Canvas, WebChat, native views) can trigger a new agent run. See `docs/gobbo-mac.md`.

## Agent commands (current)

Use the main `gobbo` CLI; it invokes canvas commands via `node.invoke`.

- `gobbo canvas present [--node <id>] [--target <...>] [--x/--y/--width/--height]`
...
- `gobbo canvas hide [--node <id>]`
- `gobbo canvas eval --js <code> [--node <id>]`
- `gobbo canvas snapshot [--node <id>]`
...
```
http://<gateway-host>:18793/__gobbo__/a2ui/
```
...
- `gobbo canvas a2ui push --jsonl <path> [--node <id>]`
- `gobbo canvas a2ui reset [--node <id>]`
...
gobbo canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```
...
- Quick smoke: `gobbo canvas a2ui push --text "Hello from A2UI"` renders a minimal v0.8 view.
...
This is intentionally separate from `gobbo-canvas://…` (which is only for serving local Canvas files into the `WKWebView`).

Suggested patterns:
- HTML: render links/buttons that navigate to `gobbo://agent?message=...`.
- JS: set `window.location.href = 'gobbo://agent?...'` for “run this now” actions.

Implementation note (important):
- In `WKWebView`, intercept `gobbo://…` navigations in `WKNavigationDelegate` and forward them to the app, e.g. by calling `DeepLinkHandler.shared.handle(url:)` and returning `.cancel` for the navigation.

Safety:
- Deep links (`gobbo://agent?...`) are always enabled.
...
- Navigation policy: allow only `gobbo-canvas://…` (and optionally `about:blank`); open `http/https` externally.
...
- Log scheme requests + resolution decisions to OSLog (subsystem `com.nickbaumann.gobbo`, category `Canvas`).
