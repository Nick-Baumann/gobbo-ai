...
- Gateway WS is configured via the app’s gateway endpoint settings (`GatewayEndpointStore`) or `gobbo gateway --port` when running locally.
...
## Dev notes
- macOS glue: `apps/macos/Sources/Gobbo/WebChatSwiftUI.swift` + `apps/macos/Sources/Gobbo/WebChatManager.swift`.
- Remote tunnel helper: `apps/macos/Sources/Gobbo/RemotePortTunnel.swift`.
