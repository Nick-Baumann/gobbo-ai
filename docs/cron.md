---
summary: "RFC: Cron jobs + wakeups for Clawd/Gobbo (main vs isolated sessions)"
read_when:
...
- Expose a stable control surface:
  ...
  - **CLI commands** (`gobbo cron ...`) to add/remove/edit/list and to debug `run`.
...
Cron persists everything under `~/.gobbo/cron/`:
- Job store: `~/.gobbo/cron/jobs.json`
- Run history: `~/.gobbo/cron/runs/<jobId>.jsonl`

You can override the job store path via `cron.store` in config.

The scheduler should never require additional configuration for the base directory (Gobbo already treats `~/.gobbo` as fixed).
...
```json5
{
  cron: {
    enabled: false,
    // optional:
    store: "~/.gobbo/cron/jobs.json",
    maxConcurrentRuns: 1
  }
}
```

You can also disable scheduling via the environment variable `GOBBO_SKIP_CRON=1`.
...
Gobbo currently serializes command execution through a global in-process queue (`src/process/command-queue.ts`) to avoid collisions.
...
- `gobbo cron edit <id> ...` (patch-by-flags, non-interactive)
- `gobbo cron rm <id>`
- `gobbo cron enable <id>` / `gobbo cron disable <id>`
- `gobbo cron run <id> [--force]` (debug)
- `gobbo cron runs --id <id> [--limit <n>]` (run history)
- `gobbo cron status` (scheduler enabled + next wake)

Additionally:
- `gobbo wake --mode now|next-heartbeat --text "<text>"` as a thin wrapper around `wake` for agents to call.
...
```bash
gobbo wake --mode next-heartbeat --text "Next heartbeat: check battery + upcoming meetings."
```
...
- Use `gobbo cron status` to confirm the scheduler is enabled and see the next wake time.
...
- Integration tests:
  - start Gateway with `GOBBO_SKIP_PROVIDERS=1`, add jobs, list/edit/remove
