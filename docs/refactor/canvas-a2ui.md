...
```
/__gobbo__/a2ui/           -> index.html
/__gobbo__/a2ui/a2ui.bundle.js -> bundled A2UI runtime
```

Serve Canvas files at `/__gobbo__/canvas/` and A2UI at `/__gobbo__/a2ui/`.
...
- Navigate to `${canvasHostUrl}/__gobbo__/a2ui/`.

### Remove bundled shells
Remove all fallback logic that serves A2UI from local bundles:
- macOS: remove custom-scheme fallback for `/__gobbo__/a2ui/`
...
1) Gateway
   ...
   - Serve them at `/__gobbo__/a2ui/` (align with existing naming).
   - Serve Canvas files at `/__gobbo__/canvas/` on `canvasHost.port`.
...
3) Tests
   - TS: verify `/__gobbo__/a2ui/` responds with HTML + JS.
