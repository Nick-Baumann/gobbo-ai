...
## Phase 0 — Foundations
- **Naming**: CLI subcommand `gobbo gateway`; internal namespace `Gateway`.
...
- **Schema tooling**:
  ...
  - `pnpm protocol:gen:swift`: generates Swift `Codable` models (`apps/macos/Sources/GobboProtocol/GatewayModels.swift`). ✅
...
- Payload types:
  ...
  - `HealthSnapshot` (match existing `gobbo health --json` fields)
...
- Add `gobbo gateway` command in CLI program:
...
- Retire `gobbo rpc` as default path; keep only if explicitly requested (documented as legacy).
...
- Ship launchd/systemd templates for `gobbo gateway`.
