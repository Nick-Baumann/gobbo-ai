...
- **Stream Errored / Conflict / status 409–515:** WhatsApp closed the socket because another session is active or creds went stale. Run `gobbo logout`, then `gobbo login`, then restart the Gateway.
- **Logged out:** Console prints “session logged out”; re-link with `gobbo login`.
...
## Helpful commands
- Start the Gateway: `gobbo gateway --verbose`
- Logout (clear creds): `gobbo logout`
- Relink (show QR): `gobbo login --verbose`
- Tail logs (default): `tail -f /tmp/gobbo/gobbo-*.log`
...
## If it keeps failing
1) `gobbo logout` → `gobbo login` (fresh QR link).
