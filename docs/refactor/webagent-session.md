...
## Persistence
- Keep passing `--session <.../.gobbo/sessions/{{SessionId}}.jsonl>` to Pi so it continues writing JSONL. The WS history reader uses the same file; no new store introduced.
