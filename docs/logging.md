...
Gobbo uses a file logger backed by `tslog` (`src/logging.ts`).

- Default rolling log file is under `/tmp/gobbo/` (one file per day): `gobbo-YYYY-MM-DD.log`
- The log file path and level can be configured via `~/.gobbo/gobbo.json`:
  - `logging.file`
  - `logging.level`
...
`gobbo gateway` supports a per-gateway style switch:
...
gobbo gateway --verbose --ws-log compact

# show all WS traffic (full meta)
gobbo gateway --verbose --ws-log full
```

## Console formatting (subsystem logging)

Gobbo formats console logs via a small wrapper on top of the existing stack:
