...
ZIP=${1:?"Usage: $0 Gobbo-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/nickbaumann/gobbo/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Gobbo-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/nickbaumann/gobbo/releases/download/v${VERSION}/"}
