...
APP_BUNDLE="${1:-dist/Gobbo.app}"
IDENTITY="${SIGN_IDENTITY:-}"
TIMESTAMP_MODE="${CODESIGN_TIMESTAMP:-auto}"
ENT_TMP_BASE=$(mktemp -t gobbo-entitlements-base)
ENT_TMP_APP=$(mktemp -t gobbo-entitlements-app)
ENT_TMP_APP_BASE=$(mktemp -t gobbo-entitlements-app-base)
ENT_TMP_BUN=$(mktemp -t gobbo-entitlements-bun)
...
# Sign main binary
if [ -f "$APP_BUNDLE/Contents/MacOS/Gobbo" ]; then
  echo "Signing main binary"; sign_item "$APP_BUNDLE/Contents/MacOS/Gobbo" "$APP_ENTITLEMENTS"
fi
...
if [ -d "$APP_BUNDLE/Contents/Resources/Relay" ]; then
  ...
  if [ -f "$APP_BUNDLE/Contents/Resources/Relay/gobbo" ]; then
    echo "Signing embedded relay"; sign_item "$APP_BUNDLE/Contents/Resources/Relay/gobbo" "$ENT_TMP_BUN"
  fi
fi
