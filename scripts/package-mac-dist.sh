...
# Output:
# - dist/Gobbo.app
# - dist/Gobbo-<version>.zip
# - dist/Gobbo-<version>.dmg

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"

"$ROOT_DIR/scripts/package-mac-app.sh"

APP="$ROOT_DIR/dist/Gobbo.app"
if [[ ! -d "$APP" ]]; then
...
ZIP="$ROOT_DIR/dist/Gobbo-$VERSION.zip"
DMG="$ROOT_DIR/dist/Gobbo-$VERSION.dmg"
NOTARY_ZIP="$ROOT_DIR/dist/Gobbo-$VERSION.notary.zip"
NOTARIZE="${NOTARIZE:-0}"
