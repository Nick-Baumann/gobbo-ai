#!/usr/bin/env bash
# Reset Gobbo like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${GOBBO_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="Gobbo.app/Contents/MacOS/Gobbo"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/Gobbo"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/Gobbo"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/Gobbo"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/com.nickbaumann.gobbo.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/gobbo-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${GOBBO_RESTART_LOG:-/tmp/gobbo-restart.log}"
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/com.nickbaumann.gobbo 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing Gobbo instances"
kill_all_gobbo
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product Gobbo"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/Gobbo.app" ]]; then
    APP_BUNDLE="/Applications/Gobbo.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/Gobbo.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/Gobbo.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/Gobbo.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set GOBBO_APP_BUNDLE to your installed Gobbo.app"
}
