...
const originalTestHome = process.env.GOBBO_TEST_HOME;

const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "gobbo-test-home-"));
process.env.HOME = tempHome;
process.env.USERPROFILE = tempHome;
process.env.GOBBO_TEST_HOME = tempHome;
...
process.on("exit", () => {
  ...
  restoreEnv("GOBBO_TEST_HOME", originalTestHome);
  try {
